...
# ╔═╡ 81055f6e-3663-41c1-8bea-047df6799fc0
clustered = begin
	nwindows = slidingwindow(split(tidytext), n = n)
	map(t -> join(t,"_"), nwindows) |> group
end
...
plotdata = begin
	...
	#ngramdata = map(keys(clustered)) do k
	#	(k, length(clustered[k]))
	#end
...
begin
	if isempty(plotdata)
		...
		gramlist = map(pr -> pr[1], plotdata)
	    gramcounts = map(pr -> pr[2], plotdata)
		thebar = bar(x=gramlist[1:maxdisplay], y=gramcounts[1:maxdisplay]) 
		Plot(thebar, graphlayout)

	end
end

# ╔═╡ d40a9d28-e73b-4f5a-aec0-098410ea942d
plotty2 =  begin
	mapped = map(keys(clustered)) do k
		(k, length(clustered[k]))
	end
	sort!(mapped, by = pr -> pr[2], rev = true)
end

# ╔═╡ 57a82184-c165-4e72-9d01-300d9fe5eed3
begin 
	ngrs = []
    for k in keys(clustered)
        push!(ngrs, (k, length(clustered[k])))
    end
	ngrs2 = map(collect(keys(clustered))) do k
		(k, length(clustered[k]))
	end
	sort!(ngrs, by = pr -> pr[2]) == sort!(ngrs2, by = pr -> pr[2])
	ngrs[1] |> typeof
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "379c869e7a751e2281f60d98754e81a546dd97ca"
